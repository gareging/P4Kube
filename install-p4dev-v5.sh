...
# This script differs from the original script in https://github.com/jafingerhut/p4-guide/blob/master/bin/install-p4dev-v5.sh
# Modifications are added due to expired keys
...
REPO_FILE="/etc/apt/sources.list.d/home:p4lang.list"

sudo bash -c "cat > $REPO_FILE <<EOF
deb [trusted=yes] http://download.opensuse.org/repositories/home:/p4lang/xUbuntu_20.04/ /
EOF"

sudo apt-key del A4666B6753C0F7D1 2>/dev/null || true
